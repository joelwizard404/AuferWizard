    ...
    try:
        with open(path, "r+b") as fh:
            ...
            if standard.verify:
                ...
                _verify_pass(fh, size, standard.passes[-1])
...
def shred_directory(root: Path, standard: Standard,
    ...
    for idx, fpath in enumerate(all_files):
        cb(ShredEvent(
            EventType.PASS_START, path=fpath,
            pass_index=idx, pass_total=total_files,
            pass_label=fpath.name,
        ))
...
def _verify_pass(fh, size: int, last_pass: Pass) -> None:
    if last_pass.pattern is None:
        return
    pattern = last_pass.pattern
    fh.seek(0)
    read = 0
    while read < size:
        ...
        for i, byte in enumerate(chunk):
            expected = pattern[(read + i) % len(pattern)]
            if byte != expected:
                raise OSError(
                    f"Verify failed at offset {read + i}: "
                    f"expected 0x{expected:02X}, got 0x{byte:02X}"
                )
        read += len(chunk)
